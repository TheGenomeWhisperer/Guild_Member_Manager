...
GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame:SetSize ( 350 , 500 );
...
GRM_UI.LoadToolFrames = function ( isManual )
    ...
    if not isManual then
        ...
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolViewSafeListButton:SetScript ( "OnEnter" , function( self )
            ...
            if countIgnored > 0 then
                if countIgnored == 1 then
                    ...
                    result = GRM.L ( "{num} players are on the safe list." );
                end
        ...
        -- Ignored ScrollFrame
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollBorderFrame:SetSize ( 200 , 275 );
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollBorderFrame:SetPoint ( "TOP" , GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollBorderFrame , "TOP" , 0 , -10 );
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollBorderFrame:SetHitRectInsets ( 1 , 50 , 1 , 1 );

        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollFrame:SetSize ( 182 , 251 );
        ...
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollFrameSilder:SetSize ( 20 , 230 );
        ...
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollFrameSilder:SetStepsPerPage ( 10 );
        ...
        GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.GRM_ToolIgnoredScrollFrameSilder:SetScript ( "OnValueChanged" , function ( self , value )
            GRM.HybridScrollOnValueChangedConfig (
                ...
                13 , 25 , GRM.BuildIgnoredScrollFrame , GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.AllIgnoredEntries
            );
        end);
...
GRM.GetIgnoredEntries = function ()
    local result = select ( 2 , GRM.GetSafePlayers ( true ) );
    ...
    for i = 1 , #GRM_UI.GRM_ToolCoreFrame.Safe do
        for j = 1 , #result do
            if result[j][1] == GRM_UI.GRM_ToolCoreFrame.Safe[i][1] then
                result[j][4] = GRM_UI.GRM_ToolCoreFrame.Safe[4];
                break;
            end
        end
    end
...
GRM.SetIgnoredValues = function ( ind , ind2 )
    ...
    if GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.AllIgnoredEntries[ind2][4] == GRM.L ( "Kick" )then
        line[3]:SetTextColor ( 1.0 , 0.84 , 0 , 1 );
    elseif GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.AllIgnoredEntries[ind2][4] == GRM.L ( "Promote" ) then
        line[3]:SetTextColor ( 1.0 , 0.84 , 0 , 1 );
    elseif GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.AllIgnoredEntries[ind2][4] ==  GRM.L ( "Demote" ) then
        line[3]:SetTextColor ( 1.0 , 0.84 , 0 , 1 );
    end
...
GRM.IgnoredHybridShiftDown = function()
    if #GRM_UI.GRM_ToolCoreFrame.GRM_ToolIgnoreListFrame.AllIgnoredEntries > 10 then
...
GRM.BuildIgnoredScrollFrame = function ( showAll , fullRefresh )
    local hybridScrollFrameButtonCount = 10;
    local buttonHeight = 25;
    local scrollHeight = 0;
        ...
        if GRM_AddonSettings_Save[GRM_G.FID][GRM_G.setPID][2][75][j][3] == 1 then
            ...
            if player[24] >= numHoursNeedsToBeUnderToNotKick and ( ( GRM_AddonSettings_Save[GRM_G.FID][GRM_G.setPID][2][55] and not GRM.IsAnyAltActive ( player[11] ) ) or not GRM_AddonSettings_Save[GRM_G.FID][GRM_G.setPID][2][55] ) then
                if not player[45] or GRM_G.playerRankID >= player[5] then      -- Ignore for scanning... but I still want a count of the ignored.
                    ...
                    table.insert ( GRM_UI.GRM_ToolCoreFrame.Safe , { player[1] , player[5] , player[9] , GRM.L ( "Kick" ) } ); -- Player { name , rankIndex , class }
                end
            end
